...
First, curves with *K*<sub>D,app</sub> around 10<sup>-6</sup> (because of nonspecific background signal, which we partially deplete, all curves rise at the higher titration concentrations so nothign gets fit to the imposed fit maximum 10^-5). These curves are fit with basically the last sample point being at 10^-6.
...
With *K*<sub>D,app</sub> around 10<sup>-7</sup>, we do seem to be catching the tails of the curves. Most of our library is in this low-affinity range
...
Same at *K*<sub>D,app</sub> of 10<sup>-9</sup>, looking pretty good. Not many curves left in this area of affinity
...
*K*<sub>D,app</sub> of 10<sup>-10</sup> or tighter -- there aren't that many curves in this regime, but they look pretty good.
...
Look at nMSR metric versus avgcoutn value, and layer on value of nMSR filtering based on 10x the global median (and percentage filtered from each background). Filter to NA fits with nMSR above this cutoff
...
We have generated binding measurements for `r round(nrow(dt[!is.na(log10Ka_TuGG)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
