---
title: "Compute per-barcode TuGG binding affinity"
author: "Tyler Starr"
date: "8/6/2021"
output:
  github_document:
...
This notebook reads in per-barcode counts from `count_variants.ipynb` for TuGG-binding Tite-seq experiments, computes functional scores for scFv TuGG-binding affiniity, and does some basic QC on variant binding functional scores.
...
#fit titration to TuGG Titeseq data for each barcode
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="lib1",Kd_TuGG]))` of our lib1 barcodes (`r round(sum(!is.na(dt[library=="lib1",Kd_TuGG]))/nrow(dt[library=="lib1",])*100,digits=2)`%) and `r sum(!is.na(dt[library=="lib2",Kd_TuGG]))` of our lib2 barcodes (`r round(sum(!is.na(dt[library=="lib2",Kd_TuGG]))/nrow(dt[library=="lib2",])*100,digits=2)`%). 
...
par(mfrow=c(2,2))
hist(log10(dt[library=="lib1" & !is.na(Kd_TuGG),TiteSeq_avgcount]+0.5),breaks=20,xlim=c(0,5),main="lib1",col="gray50",xlab="average cell count across concentration samples")
hist(log10(dt[library=="lib1" & is.na(Kd_TuGG),TiteSeq_avgcount]+0.5),breaks=30,add=T,col="red")

hist(log10(dt[library=="lib2" & !is.na(Kd_TuGG),TiteSeq_avgcount]+0.5),breaks=20,xlim=c(0,5),main="lib2",col="gray50",xlab="average cell count across concentration samples")
hist(log10(dt[library=="lib2" & is.na(Kd_TuGG),TiteSeq_avgcount]+0.5),breaks=30,add=T,col="red")

hist(dt[library=="lib1" & !is.na(Kd_TuGG),TiteSeq_min_cell_filtered],breaks=5,main="lib1",col="gray50",xlab="number of sample concentrations below cutoff cell number",xlim=c(0,10))
hist(dt[library=="lib1" & is.na(Kd_TuGG),TiteSeq_min_cell_filtered],breaks=20,add=T,col="red")

hist(dt[library=="lib2" & !is.na(Kd_TuGG),TiteSeq_min_cell_filtered],breaks=5,main="lib2",col="gray50",xlab="number of sample concentrations below cutoff cell number",xlim=c(0,10))
hist(dt[library=="lib2" & is.na(Kd_TuGG),TiteSeq_min_cell_filtered],breaks=20,add=T,col="red")
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 1e-10 & dt$Kd_TuGG < 1.2e-10)[1])
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 1e-10 & dt$Kd_TuGG < 1.2e-10)[2])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 1e-10 & dt$Kd_TuGG < 1.2e-10)[1])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 1e-10 & dt$Kd_TuGG < 1.2e-10)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 3.1e-11 & dt$Kd_TuGG < 3.2e-11)[1])
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 3.1e-11 & dt$Kd_TuGG < 3.2e-11)[2])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 3.1e-11 & dt$Kd_TuGG < 3.2e-11)[1])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 3.1e-11 & dt$Kd_TuGG < 3.2e-11)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 1e-11 & dt$Kd_TuGG < 2e-11)[1])
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 1e-11 & dt$Kd_TuGG < 2e-11)[2])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 1e-11 & dt$Kd_TuGG < 2e-11)[1])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 1e-11 & dt$Kd_TuGG < 2e-11)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 3.1e-12 & dt$Kd_TuGG < 3.2e-12)[1])
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 3.1e-12 & dt$Kd_TuGG < 3.2e-12)[2])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 3.1e-12 & dt$Kd_TuGG < 3.2e-12)[1])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 3.1e-12 & dt$Kd_TuGG < 3.2e-12)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 1e-12 & dt$Kd_TuGG < 4e-12)[1])
plot.titration(which(dt$library=="lib1" & dt$Kd_TuGG > 1e-12 & dt$Kd_TuGG < 4e-12)[2])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 1e-12 & dt$Kd_TuGG < 4e-12)[1])
plot.titration(which(dt$library=="lib2" & dt$Kd_TuGG > 1e-12 & dt$Kd_TuGG < 4e-12)[2])
```
...
threshold <- 20
par(mfrow=c(1,1))
plot(log10(dt[,TiteSeq_avgcount]),dt[,nMSR_TuGG],main="",pch=19,col="#00000010",xlab="average cell count (log10)",ylab="nMSR",xlim=c(0,3.5),ylim=c(0,0.5))
...
```{r convert_log10Ka}
dt[,log10Ka := -log10(Kd_TuGG),by=c("barcode","library")]
```
...
p1 <- ggplot(dt[!is.na(log10Ka),],aes(x=variant_class,y=log10Ka))+
  ...
  ggtitle("huTuGG, log10(Ka)")+xlab("variant class")+theme(axis.text.x=element_text(angle=-90,hjust=0))+
